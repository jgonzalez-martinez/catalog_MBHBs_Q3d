...
def R_z_Q3d_base(z):
    return np.where(z <= 3.5, 0.43*z, -0.18*z + 2.12)  # [yr^-1 per unit z]

#Target total rate (per year). Barausse MBHB catalog, extrapolated
TARGET_RATE_PER_YEAR_Q3D = 18.4  # yr^-1

#Normalisation so the integral of Rz_Q3d dz = TARGET_RATE_PER_YEAR_Q3D 
zmin, zmax = 0.0, 12.0  
z_grid = np.linspace(zmin, zmax, 1000)
raw_integral = np.trapz(R_z_Q3d_base(z_grid), z_grid)  
f_norm = TARGET_RATE_PER_YEAR_Q3D / raw_integral      

def R_z_Q3d(z):
    return f_norm * R_z_Q3d_base(z)  # normalised dN/dz [yr^-1]
...
z_grid = np.linspace(0, 12, 1000) 

# Merger rate density
R_vals = R_z_Q3d(z_grid)
...
def sample_redshifts_from_Rz(N, z_min=0, z_max=12.0, grid_size=1000):
    ...
    #dN_dz = R_z_Q3d(z_vals) * dVc_dz / (1.0 + z_vals)
    dN_dz = R_z_Q3d(z_vals)
...
def sample_spins(N, alpha=5, beta_param=2, seed=None):
...
def generate_synthetic_MBHB_population(N, geoctime_i, geoctime_f,                                       
                                       ...
                                       m_min=1e4, m_break=1e6, m_max=1e7,
                                       q_min=0.1, q_max=1.0,
                                       spin_alpha=5, spin_beta=2,
                                       # NEW options
                                       z_min=0, z_max=12.0, grid_size=1000):
    ...
    # Spins
    chi1, chi2 = sample_spins(N, spin_alpha, spin_beta)
